...
## preUploadTasks
*string[]*: Run these hooks before each local-to-remote file upload. Hooks run in order and stop on first failure.
...
Hooks run in order. Post hook failures are logged but do not roll back a completed file upload.
